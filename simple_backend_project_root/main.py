...
async def run_workflow(
    ...
    # 对 A/B 级 + 高潜 C 级进行二次搜索
    target_leads = [
        l for l in scored
        if l.get("customer_grade") in ("A", "B") or
        (l.get("customer_grade") == "C" and l.get("customer_value_score", 0) >= 50)
    ]
    ...
    if target_leads:
        ...
        # 更新推进就绪分（不重新算客户匹配分）
        from scorer import _score_readiness
        for lead in merged:
            readiness = _score_readiness(lead)
            lead.update(readiness)
            lead["data_completeness_score"] = readiness["readiness_score"]
            lead["second_search_status"] = readiness["second_search_status"]
            lead["next_action_stage2"] = readiness["next_action_stage2"]
            lead["final_grade"] = lead.get("customer_grade", "D")
            lead["grade"] = lead.get("customer_grade", "D")
            lead["total_score"] = lead.get("customer_value_score", 0)
    ...
    if output_feishu:
        if scored:
            print(f"[DEBUG] write_feishu 调用: table_id={feishu_table_id}, leads={len(scored)}")
            result = await write_feishu(scored, table_id=feishu_table_id)
            print(f"飞书: {result}")
            if isinstance(result, dict) and result.get("error"):
                raise RuntimeError(result["error"])
        else:
            print(f"⚠️ 无 lead 数据，跳过飞书写入")
...
def main():
    ...
    parser.add_argument(
        "--feishu-table-id",
        default=None,
        help="写入指定多维表 table_id（与 Base App 下新建表对应）",
    )
